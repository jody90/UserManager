...
SPRING_OPTS="-Xms256m -Xmx512m -server -Xloggc:./logs/gc.log -verbose:gc -XX:+PrintGCDetails -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=./logs -Dspring.config.location=./config/application.properties -Dspring.profiles.active=%ENV%"
...
start)
	...
	if [ $ENV == '' ]
	then 
	echo 'Missing profile [development, production]'
	exit 1
	fi
